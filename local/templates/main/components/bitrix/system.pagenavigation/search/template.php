...
<div class="search__results-tabs__body-item__pagenav pageBlock" data-num="<?=$arResult['NavNum']?>">
    <?if($arResult['NavPageNomer'] != 1){?>
        <a href="<?=$arResult['sUrlPath']?>?page=<?=$arResult['NavPageNomer']-1?>" data-link="<?=$arResult['sUrlPath']?>?page=<?=$arResult['NavPageNomer']-1?>" class="arrow prev pageAjaxSearch" data-attr="←" data-page="<?=$arResult['NavPageNomer']-1?>">
    ...
    <?for ($i=1;$i<=$arResult['NavPageCount'];$i++){?>
        ...
        <?}else{?>
            <?if($arResult['NavPageCount'] <= 6){?>
                <a href="<?=$arResult['sUrlPath']?>?page=<?=$i?>" data-link="<?=$arResult['sUrlPath']?>?page=<?=$i?>" class="link pageAjaxSearch" data-page="<?=$i?>" data-attr="<?=$i?>"><?=$i?></a>
            <?}else{?>
    ...
    <?if($arResult['NavPageNomer'] != $arResult['NavPageCount']){?>
        <a href="<?=$arResult['sUrlPath']?>?page=<?=$arResult['NavPageNomer']+1?>" data-link="<?=$arResult['sUrlPath']?>?page=<?=$arResult['NavPageNomer']+1?>" class="arrow next pageAjaxSearch" data-attr="→" data-page="<?=$arResult['NavPageNomer']+1?>">
            <svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
