...
$course = $courseEntity::getRow([
    'select' => [
        ...
        'CERT_TEMPLATE_VALUE' => 'CERT_TEMPLATE.VALUE'
    ],
    'filter' => [
        'IBLOCK_ID' => COURSES,
        'CODE' => $_GET['course'],
...
if ($moduleIds){
    ...
    while ($userModule = $rsModuleUser->fetch()){
        ...
        if ($userModule['UF_COMPLETED']){
            $progress = 100;
        } else {
            $progress = $userModule['UF_PROGRESS'] ?: 0;
        }
        $totalModuleProgress += $progress;
    }

    $courseProgress = round(($totalModuleProgress / count($moduleIds)), 1);
}
